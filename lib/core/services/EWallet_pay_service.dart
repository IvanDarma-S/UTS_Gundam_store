...
void _log(String tag, String message) {
  debugPrint('[GundamStore/$tag] $message');
}
...
class EWalletService {
  static final EWalletService _instance = EWalletService._();
  factory EWalletService() => _instance;
  EWalletService._();
  ...
  Future<void> init() async {
    _log(_tag, ' Inisialisasi EWalletService...');
  ...
  void _handleUri(Uri uri, {bool isColdStart = false}) {
    ...
    if (uri.scheme != 'gundamstore') {
      _log(
        _tag,
        '⏩ Diabaikan — bukan skema gundamstore (scheme=${uri.scheme})',
      );
      return;
    }
  ...
  static String buildDeeplinkUrl({
    ...
    const scheme = 'GunplaPay';
    const host = 'pay';
    final desc = (description != null && description.isNotEmpty)
        ? description
        : 'Order #$orderId';
    const callbackUrl = 'gundamstore://payment-callback';

    _log(_tag, ' Membangun deeplink URL:');
    _log(_tag, 'merchant_id : MCH_GUNDAM_STORE');
    _log(_tag, 'merchant_name: gundam Store');
    ...
    final uri = Uri(
      ...
      host: host,
      queryParameters: {
        'merchant_id': 'MCH_GUNDAM_STORE',
        'merchant_name': 'gundam Store',
